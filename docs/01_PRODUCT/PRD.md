...
A capacidade Habits V1 ? owner-scoped e registra defini??es de h?bitos e fatos bin?rios de conclus?o di?ria. A autoridade de Feature IDs ? `FEATURE_CATALOG.md`. O escopo aprovado de V1 cont?m HAB-001 e HAB-002; HAB-003, HAB-004 e HAB-005 permanecem DEFERRED.

## HAB-001 ? Cadastro de h?bitos

A defini??o Habit cont?m `id`, `owner_id`, `name`, descri??o opcional, `active` e timestamps t?cnicos. O owner vem da autentica??o; o nome ? obrigat?rio e ?nico dentro do owner; o h?bito nasce ativo; o owner pode desativar e reativar. Desativa??o preserva conclus?es hist?ricas. V1 n?o permite rename, delete, soft delete, `deleted_at`, frequ?ncia, quantidade, atributos, categorias, tags, unidades, agendas, rotinas, lembretes ou notifica??es. `attribute_code` n?o ? reservado em V1.

## HAB-002 ? Checklist di?rio

`HabitCompletion` ? um fato bin?rio: a exist?ncia do registro afirma que o owner concluiu o h?bito na data civil indicada. No m?ximo um fato existe por `(owner_id, habit_id, record_date)`. Marcar novamente ? idempotente; o owner pode remover para corrigir. N?o h? `completed`, quantidade, dura??o, alvo ou linhas sint?ticas de dia perdido. H?bito inativo n?o aceita nova conclus?o, mas conclus?es hist?ricas continuam consult?veis.

`record_date` ? uma data civil expl?cita fornecida pelo cliente, n?o derivada de UTC. `created_at` ? timestamp t?cnico UTC. Regras de data futura dependentes de fuso do owner ficam deferred at? existir perfil de fuso.

## Rastreabilidade RF-HAB

- RF-HAB-001 ? Cadastro de H?bito ? HAB-001.
- RF-HAB-002 ? Gerenciamento de Rotinas ? DEFERRED; n?o ? HAB-002.
- RF-HAB-003 ? Registro de Execu??o do H?bito ? HAB-002, com sem?ntica bin?ria de `HabitCompletion`.
- RF-HAB-004 ? Controle de Frequ?ncia ? HAB-004, DEFERRED.
- RF-HAB-005 ? Controle de Streaks ? HAB-003, DEFERRED.
- RF-HAB-006 ? Consulta ao Hist?rico de Execu??es ? HAB-002.
- RF-HAB-007 ? Visualiza??o de Estat?sticas ? HAB-005, DEFERRED.
- RF-HAB-008 ? Visualiza??o da Evolu??o dos H?bitos ? DEFERRED / CROSS-DOMAIN FUTURE.
- RF-HAB-009 ? Integra??o com Game Engine ? DEFERRED / NOT AUTHORIZED.
- RF-HAB-010 ? Consulta Consolidada dos H?bitos ? HAB-001 quando significar listar defini??es do owner.

Habits V1 ? owner-private: activity existence ? YES; external exposure, progression eligibility, Logos integration, Noema/AI access e generic/event dispatch s?o NO. LifeOS n?o calcula XP a partir de HabitCompletion. N?o h? API, schema, migration ou integra??o autorizada por esta se??o.
